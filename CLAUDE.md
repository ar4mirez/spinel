...
- Definition of done for engine work is a ruby/spec delta: name the spec files or directories that newly pass. `scripts/spec.sh core/array` runs one directory. Never mark a spec as "expected failure" to make a slice green; skip it with a reason in `spec/tags/<path>_tags.txt`, one `fails(reason):full description` per line — mspec's own format, and the reason is required. Read `spec/tags/README.md` before writing one; do not invent a format.
