...
Phase 1. Progress is measured one way: ruby/spec pass counts per directory, published in [`bench/spec-status.md`](bench/spec-status.md) by CI — one row per directory, regenerated by `scripts/spec-status.sh` and never typed by hand. The corpus is in the tree and `scripts/spec.sh [dir]` counts it. There is a VM now — bytecode, a calling convention, classes and constants, the exception unwinder, a regex engine, and a core library in Ruby — and the table's `language/` row is the one this phase is measured by. `spinel run app.rb` runs Ruby. Nothing is marked as an expected failure: an example either passes, fails, or names the construct that blocks it, and `scripts/verify-passes.rb` re-runs every claimed pass on a real Ruby so that a pass means what it says.
