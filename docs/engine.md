...
A method carries its visibility, landed with [#161](https://github.com/ar4mirez/spinel/issues/161). It sits on the `Method` the caches memoise rather than beside it, so a warm call site checks `public`/`private`/`protected` for free; the price is that `private :m` has to bump the class serial exactly as redefining `m` does, which `Classes::set_visibility` does. The visibility a bare `private` sets for the `def`s below it is a different thing and lives on the *frame* — CRuby's `CREF_SCOPE_VISI` — because it belongs to one execution of one body: a block shares it, a method body resets it, and reopening a class starts public again.
