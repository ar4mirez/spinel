...
- `core/kernel.rb`, `core/object.rb`, minimal `Integer`, `String`, `Array`, `Hash`, `Symbol` in Ruby, enough to run the specs above. *Check:* `spinel run hello.rb` and the phase's spec directories. **Done:** `core/*.rb` loaded by `spinel-core`, a growable two-slot `Array`, `Class#allocate` per class, and `spinel run`; `language/` 545 to 619, the corpus 713 to 1,023.
