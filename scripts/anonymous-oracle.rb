...
# The oracle behind `crates/spinel-vm/tests/anonymous.txt`.
#
# `Class.new` and `Module.new` are four rules that read like one, and three of
# them are counter-intuitive: the block moves `self` but not the lexical scope,
# only the *first* constant assignment names a class, and `inherited` fires
# before the block rather than after despite what ruby/spec's example is called.
# Each line here is CRuby's answer, measured, so the expectations in
# `crates/spinel-vm/tests/anonymous.rs` are not one engineer's belief.
#
#   ruby scripts/anonymous-oracle.rb --check      # CI: agree, or fail
#   ruby scripts/anonymous-oracle.rb --generate   # rewrite the `#=>` results
#
# Format: `<snippet>  #=> <result>`, one per line, `#` comments and blank lines
# ignored. A snippet that raises records `!Class: message` rather than a value,
# because half of what this slice had to get right is which error Ruby picks.
# Addresses are normalised to `0xXX`: the shape of `#<Class:0x...>` is the rule,
# the address is not.

TABLE = File.expand_path("../crates/spinel-vm/tests/anonymous.txt", __dir__)
SEPARATOR = "  #=> "
