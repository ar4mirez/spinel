...
struct Cli {
    ...
    /// The platform the `platform_is` guards are answered for: `darwin`,
    /// `linux`, `windows`. Defaults to the host.
    ///
    /// It exists for `--by-directory`, whose table is committed. Guards make the
    /// counts platform-dependent — `platform_is :linux` runs a different set of
    /// examples on each — so a progress bar generated on a laptop and checked on
    /// CI would differ every time. `scripts/spec-status.sh` pins one value so
    /// the file is reproducible anywhere; a plain run still answers for the
    /// machine it is on, which is what someone debugging locally wants.
...
fn main() -> ExitCode {
    ...
    // them.
    if cli.by_directory {
        print_status(&per_directory, &totals, &target.platform);
...
fn print_status(per_directory: &BTreeMap<String, Counts>, totals: &Counts, platform: &str) {
    ...
    println!();
    // The platform is part of the measurement, not decoration: ruby/spec's
    // `platform_is` guards run a different set of examples on each one, so a
    // table generated on a laptop and checked on CI would disagree for a reason
    // that has nothing to do with progress.
    println!(
        "Guards are answered for `{platform}`, which is what `scripts/spec-status.sh` \
         pins so that this file is the same wherever it is generated."
    );
    println!();
